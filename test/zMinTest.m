...
resultBaseFolder = fullfile('../results/zMinTest');
% paramTable = table(...
...
zMinSet = 0.01:0.005:0.1;
outputFileName = cell(size(zMinSet, 1), 1);
for k = 1:size(zMinSet, 2)
	outputFileName{k} = fullfile(resultBaseFolder, ...
		['zMinTest_0', num2str(zMinSet(k)*1000), '.xlsx']);
end

for i = 1:size(zMinSet, 1)
	if exist(outputFileName{i}, 'file')
		delete(outputFileName{i});
	end
end
...
sumAbsLocError = zeros(size(zMinSet, 2), 1);
sumAbsLocErrorCI = zeros(size(sumAbsLocError));
sumAbsOrientError = zeros(size(zMinSet, 2), 3);
...
resultTable = repmat(resultTable, size(zMinSet, 2), 1);
...
resultCItable = repmat(resultCItable, size(zMinSet, 2), 1);
...
for k = 1:size(zMinSet, 2)
	...
	for i = 1:repetitions
		...
		[vSet, xyzPoints, reprojectionErrors, ...
			...
			windowSize, groundTruthPoses, c, bundleAdjustmentAbsoluteTolerance);
	...
	params = table(repetitions, computeRelativeScaleBeforeBundleAdjustment, ...
		...
		performWindowedBundleAdjustment, windowSize);
...
figure;
plotWithErrorBar(tmpLocErrorResult', tmpLocErrorCI');
...
figure;
plotWithErrorBar(tmpOrientErrorXresult', tmpOrientErrorXCI');
...
figure;
plotWithErrorBar(tmpOrientErrorYresult', tmpOrientErrorYCI');
...
figure;
plotWithErrorBar(tmpOrientErrorZresult', tmpOrientErrorZCI');
